...
void main() {
  test(
    'with prefs OFF and prefs ON discovery returns same list length',
    () async {
      ...
      final List<Restaurant> listOff =
          (resultOff as Success<List<Restaurant>>).data;
      final List<Restaurant> listOn =
          (resultOn as Success<List<Restaurant>>).data;

      final int n = listOff.length;
      expect(
        listOn.length,
        n,
        reason: 'Preferences must not filter; list length must stay $n',
      );
      ...
      expect(
        idsOn,
        idsOff,
        reason: 'Same restaurants must appear; only order may differ',
      );
    },
  );
}
